    ...
    <section class="glass rounded-[32px] p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-3xl font-black">Quản lý nạp tiền</h1>
            </div>
            <form method="get" class="flex flex-wrap gap-3">
                ...
                <select class="premium-input w-48" name="status">
                    ...
                    @foreach(['pending' => 'Đang xử lý', 'paid' => 'Thành công', 'failed' => 'Thất bại'] as $value => $label)
                        <option value="{{ $value }}" @selected(($filters['status'] ?? '') === $value)>{{ $label }}</option>
                    @endforeach
                </select>
                <button class="rounded-2xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-5 py-3 font-black text-white shadow-glow">Lọc</button>
            </form>
        </div>
    </section>
                ...
                <tbody class="divide-y divide-white/10">
                    @forelse($orders as $order)
                        @php
                            ...
                            $bank = $order->metadata['bank_code'] ?? ($order->metadata['bank_name'] ?? 'Chưa cập nhật');
                            $statusMap = [
                                'paid' => ['Thành công', 'bg-emerald-400/10 text-emerald-100'],
                                'failed' => ['Thất bại', 'bg-rose-400/10 text-rose-100'],
                                'pending' => ['Đang xử lý', 'bg-amber-300/10 text-amber-100'],
                            ];
                            [$statusLabel, $statusClass] = $statusMap[$order->status] ?? ['Khác', 'bg-white/10 text-white'];
                        @endphp
                        <tr class="text-slate-300 transition hover:bg-white/[.04]">
                            ...
                            <td class="font-semibold text-emerald-100">{{ number_format($order->amount_vnd, 0, ',', '.') }}đ</td>
                            <td>
                    ...
                    @empty
                        <tr><td colspan="8" class="py-8 text-center text-slate-400">Chưa có giao dịch nạp tiền.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
