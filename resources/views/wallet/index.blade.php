...
<div class="space-y-8">
    ...
    @if($bankAccount)
        <section class="glass rounded-[32px] p-6">
            ...
            <form method="post" action="{{ route('wallet.withdraw') }}" class="grid gap-4 sm:grid-cols-[1fr_auto]">
                @csrf
                <input type="hidden" name="bank_account_id" value="{{ $bankAccount->id }}">
                <div class="grid gap-2">
                    <input class="premium-input" name="amount_vnd" type="text" inputmode="numeric" autocomplete="off" pattern="[0-9.]*" data-currency-input value="{{ old('amount_vnd') }}" placeholder="Tối thiểu 100.000 đ">
                ...
                <button class="self-start rounded-2xl bg-gradient-to-r from-emerald-400 to-violet-500 px-6 py-4 font-black shadow-glow transition hover:-translate-y-1">Tạo yêu cầu</button>
            </form>
        </section>
    @endif
                    ...
                    @php
                        ...
                        if (in_array($entry->type, ['withdrawal_hold', 'withdrawal_completed', 'withdrawal_payout'], true) && $entry->reference instanceof \App\Models\WithdrawalRequest) {
                            $entryMemo = match ($entry->reference->status) {
                                'approved' => 'Đã hoàn tất việc rút tiền',
                                'rejected' => 'Tạm giữ yêu cầu rút tiền',
                                default => 'Tạm giữ yêu cầu rút tiền',
                            };
                            $entryTypeLabel = $entry->reference->status === 'approved' ? 'Rút tiền hoàn tất' : 'Tạm giữ rút tiền';
                        }
